  ...
  componentWillReceiveProps (nextProps) {
    ...
    select(this).transition().duration(transitionDuration).ease(easeLinear).tween('attr.scale', () => {
      const interpolators = scaleProps.map(scaleProp => {
        const [nextDomainMin, nextDomainMax] = nextProps[scaleProp].domain()
        const minInterpolator = interpolateNumber(this.state[`${scaleProp}Min`], nextDomainMin)
        const maxInterpolator = interpolateNumber(this.state[`${scaleProp}Max`], nextDomainMax)
