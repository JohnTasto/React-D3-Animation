...
export default class BarChart extends React.Component {
  ...
  buildBandScale (domain, range) {
    return scaleBand().domain(domain).rangeRound(range)
  }
  ...
  render () {
    ...
    const yExtent = Object.keys(data).map(label => data[label]).reduce(([min, max], curr) => [Math.min(curr, min), Math.max(curr, max)], [Infinity, -Infinity])
    const xScale = scaleBand().domain(Object.keys(data).sort()).range([0, width])
    const yScale = scaleLinear().domain([0, yExtent[1]]).range([height, 0])
